...
// comparison, the route a lead passes to its children (mirrors dsh-subagent
// parentAgentOptionsForDelegation, kernel-check K04), delegation depth (mirrors
// delegationDepthOf, K05), explicit-route detection and route preflight through
...
// The route a child inherits from this agent, read the way dsh-subagent's
// parentAgentOptionsForDelegation does: the latest request header wins (it
// carries request-time model selection, e.g. a model switched in the UI);
// creation options apply only before the first request.
export function leadRoute(agent) {
  let config;
  try {
    config = agent?.session?.requestHeader?.()?.config;
  } catch {
    config = undefined;
  }
  if (config === undefined || config === null) return observedRoute(agent?.options);
  return observedRoute({ provider: config.provider, model: config.model, reasoningEffort: config.reasoningEffort });
}
