...
import { defaultRoute, routeLabel, entryLabel, text, observedRoute, leadRoute, observedLabel, sameRoute, pinnedMatch, plannedLabel, agentKey, depthOf, messageOf } from "./routes.js";
...
function successReport(ctx, exec, store, state) {
  ...
  const lead = leadRoute(exec?.agent);
  let verified = false;
...
export function installTeammateHooks(ctx, state, book, life, teammateCall, track) {
  ...
  const onPostExecute = async (exec, result, next) => {
    ...
    try {
      // A call made from run_code hands its program the value, never the text,
      // so there (exec.parent is the run_code execution) the note rides as a
      // context the host defers to the run_code result; so does a note whose
      // value was replaced or whose result was blocked.
      if (exec?.parent === undefined && decision?.kind === "accept" && !Object.hasOwn(decision, "value")) {
        const content = decision.content ?? result?.content ?? [];
        return { ...decision, content: [...content, { type: "text", text: note }] };
      }
